...
test("Verify function getX, getting the value => 200, from the constructor ", () => {
  const ball = new Ball(200, 100, 8, 3, -3);
  const gettingX = ball.getX();
  expect(gettingX).toBe(200);
});

test("Verify function getY, getting the value => 100,from the constructor ", () => {
  const ball = new Ball(200, 100, 8, 4, 3, -3);
  const gettingY = ball.getY();
  expect(gettingY).toBe(100);
});

test("Verify function getRadius, getting the value => 8,from the constructor ", () => {
  const ball = new Ball(200, 100, 8, 4, 3, -3);
  const gettingRadius = ball.getRadius();
  expect(gettingRadius).toBe(8);
});

test("Verify that function moveBall is moving dx, getting the value => 3,from the constructor ", () => {
  const ball = new Ball(200, 100, 8, 4, 3, -3);
  ball.moveBall();
...
test("Verify that executing the function moveBall makes move dy value, getting the value => -3", () => {
  const ball = new Ball(200, 100, 8, 4, 3, -3);
  ball.moveBall();
...
test("Verify that executing the function  moveBall makes move the x position when rightArrow is true, getting the value => 205", () => {
...
test("Verify that executing the function moveBall makes move a negative x postion when leftArrow is true, getting the value => 195", () => {
  const paddle = new Paddle(cvs, 80, 20, 30, 5);
  const ball = new Ball(
    200,
    100,
...
test("Verify that executing the function moveBall makes move position y in a postive way when upArrow is true, getting the value => -3", () => {
  const paddle = new Paddle(cvs, 80, 20, 30, 5);
  const ball = new Ball(
    200,
    0,
