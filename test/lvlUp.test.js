...
test("Verify that function levelUp, getting the value => false, When you are in any Level except When you surpass level 3 ", () => {
...
test("Verify that function levelUp, getting the value => true, When finished the LvL 3 ", () => {
  const ball = new Ball(10, 12, 50, 4, 3, -3, sound);
  const paddle = new Paddle(cvs, 300, 20, 30, 5);
  const brick = new Bricks(ctx, 1, 5, 55, 20, 20, 20, 40, "#2e3548", "#FFF");
  const drawball = new drawBall(ctx, cvs, paddle.getY(), ball);
...
test("Verify that function levelUp when all bricks were broken in level 1", () => {
  const ball = new Ball(10, 12, 50, 4, 3, -3, sound);
  const paddle = new Paddle(cvs, 300, 20, 30, 5);
