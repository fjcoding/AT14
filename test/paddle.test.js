...
test("Verify function getX, getting the x value => 150, from the object Paddle ", () => {
  const padle = new Paddle(cvs, 100, 20, 30, 5);
  const gettingX = padle.getX();
  expect(gettingX).toBe(150);
});

test("Verify function getY, getting the y value => 150, from the object Paddle ", () => {
  const padle = new Paddle(cvs, 100, 20, 30, 5);
  const gettingY = padle.getY();
  expect(gettingY).toBe(150);
});

test("Verify function getdX, getting the value => 5, from the object Paddle", () => {
  const padle = new Paddle(cvs, 100, 20, 30, 5);
